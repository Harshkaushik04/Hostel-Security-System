...
# --- 1. Hardware & Models ---
# Explicitly move PyTorch to the GPU if it exists
...
# --- 4. The Video Processing Worker (Optimized) ---
def process_stream(camera_name: str):
    ...
    # Aggressively disable OpenCV buffering via FFmpeg flags
    os.environ["OPENCV_FFMPEG_CAPTURE_OPTIONS"] = "rtsp_transport;tcp|fflags;nobuffer|flags;low_delay"
    cap = cv2.VideoCapture(rtsp_url)
    
    # --- OPTIMIZATION 1: The Frame Dropper Queue ---
    ...
    def reader_thread():
        while active_streams.get(camera_name, False):
            ...
            # Clear the queue to absolutely ensure we only keep the newest frame
            try:
    ...
    # Start the fast reader in the background
    reader = threading.Thread(target=reader_thread, daemon=True)
    reader.start()

    # Process maximum twice a second to free up CPU
    ...
    while active_streams.get(camera_name, False):
        ...
        # Time-based throttling instead of frame-count skipping
        current_time = time.time()
        ...
        # Math is exponentially faster on smaller images. Cap width at 720.
        h, w = frame.shape[:2]
        if w > 720:
            scale = 720.0 / w
        ...
        # --- OPTIMIZATION 3: Grayscale PyZbar ---
            ...
            try:
                ...
                if boxes is not None:
                    for box in boxes:
                        ...
                        # Filter out glitchy/tiny false positive boxes
                        if face.size == 0 or face.shape[0] < 20 or face.shape[1] < 20:
                            continue

                        face_resized = cv2.resize(face, (160, 160))
                        
                        # --- OPTIMIZATION 4: Hardware Inference ---
